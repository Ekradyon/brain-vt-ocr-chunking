...
## `oid` (obligatorio)
- Tipo: `int`.
- Funcion: identificador principal del proceso.
- Regla: el servicio usa el mismo valor como `documento_id` interno para persistencia.
- Si no se envia: `422`.
...
- Uso recomendado:
  - `nombre_documento`
  - `metadata_documento` (paginas, idioma, etc.)
  - `ruta_pdf`
...
## `overwrite.enabled`
- `true`: borra embeddings existentes del documento (`oid`) e inserta de nuevo.
- `false`: si existen embeddings del documento, falla con `DUPLICATE_EMBEDDINGS`.
